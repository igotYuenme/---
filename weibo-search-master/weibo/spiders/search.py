...
def parse(self, response):
    ...
    # 1. 保存完整的响应信息（包括头部）
    debug_file = os.path.join(debug_dir, f'resp_{safe_keyword}_p{page}_INFO.txt')
    ...
    print(f'[DEBUG] 响应信息已保存至: {debug_file}')

    # 2. 仍然保存原始HTML文件以便用浏览器查看
    html_file = os.path.join(debug_dir, f'resp_{safe_keyword}_p{page}.html')
